...
const Routes = (shoesDb, shoesFE ) => {
    ...
    const home = async (req, res, next) => {
        ...
        let shoes = []
        await axios.get('http://localhost:5000/api/shoes')
            .then((response) => {
    ...
    const viewShoe = async (req, res, next) => {
        ...
        .get(`http://shoe-catalogue.localhost:5000/api/shoe/${id}`)
            ...
            if(password.length < 8){
                ...
                if(emailPattern.test(email)){
                    ...
                    let data = {"name": name, "lastname": lastname, "email": email, "password": password}
                        const response = await fetch("http://shoe-catalogue.localhost:5000/api/auth/signup", {
                        method: 'POST',
                        mode: 'cors',
                        cache: 'no-cache',
        ...
        if(!email){
            ...
            if(password.length < 8){
                ...
                if(emailPattern.test(email)){
                    ...
                    const response = await fetch("http://shoe-catalogue.localhost:5000/api/auth/signin", {
                        method: 'POST',
                        mode: 'cors',
                        cache: 'no-cache',
                        credentials: 'same-origin',
    ...
    const postShoeDb = async (req, res) => {
        ...
        let imageAsBase64 = fs.readFileSync(`./public/img/${image}`, 'base64');
            ...
            await fetch("http://localhost:5000/api/shoes/add-shoe", {
              method: 'POST',
              mode: 'cors',
              cache: 'no-cache',
              credentials: 'same-origin',
    ...
    const search = async (req, res) => {
        ...
        if(brand){
            await axios.get(`http://localhost:5000/api/shoes/brand/${brand}`)
            .then((response) => {
                if(response?.data?.status === 'success'){
                    shoes = response?.data?.shoes
                }
            ...
            await axios.get(`http://localhost:5000/api/shoes/size/${size}`)
            .then((response) => {
                if(response?.data?.status === 'success'){
                    shoes = response?.data?.shoes
                }
            ...
            await axios.get(`http://localhost:5000/api/shoes/brand/${brand}/size/${size}`)
            .then((response) => {
                if(response?.data?.status === 'success'){
                    shoes = response?.data?.shoes
                }
